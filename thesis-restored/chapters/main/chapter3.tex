...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{graphics/figure3-6-offline-pipeline.png}
...
Với truy vấn thô $q$ từ người dùng, bộ phân tích truy vấn thực hiện ba bước:

\begin{enumerate}
  \item \textbf{Phân loại ý định (Intent Classification):} Phân loại truy vấn
        thành một trong sáu loại ý định gồm Xử phạt (Penalty), Định nghĩa
        (Definition), Thủ tục (Procedure), Yêu cầu (Requirement), Tham chiếu
        (Reference), Chung (General), thông qua regex tiếng Việt với LLM fallback.

  \item \textbf{Mở rộng viết tắt (Abbreviation Expansion):} Mở rộng các viết tắt
        chuyên ngành, ví dụ: TNHH $\to$ trách nhiệm hữu hạn, CTCP $\to$ công ty
        cổ phần.

  \item \textbf{Trích xuất thực thể giống (Seed Entity Extraction):} Trích xuất
        các tham chiếu điều khoản, tham chiếu luật và từ khóa pháp lý.
\end{enumerate}

Các đầu ra này được đưa trực tiếp vào các tầng truy xuất: Tầng 1 sử dụng truy vấn
đã mở rộng cho duyệt cây, Tầng 2 sử dụng thực thể giống để khởi tạo PageRank và
nhãn ý định để điều chỉnh trọng số cạnh (1,0 cho quan hệ chính, 0,5 cho quan hệ
khác). Hình~\ref{fig:query-analysis} minh họa ba bước phân tích với ví dụ cụ thể.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{graphics/figure3-4-query-analysis.png}
...
% -------------------------------------------------------------------
\subsection{Low-level (Entity-specific)}
...
% -------------------------------------------------------------------
\subsection{High-level (Conceptual)}
...
% -------------------------------------------------------------------
\subsection{Hợp nhất Low-level và High-level}
...
hệ của Knowledge Graph. Các quan hệ mạnh (STRONG), bao gồm references, requires,
depends\_on, được duyệt theo hai hướng:
...
\begin{enumerate}
  ...
  \item \textbf{Hệ số phạt (Penalty):}
        \begin{itemize}
          ...
          \item Noise penalty ($0{,}2\times$): Các điều khoản có tỷ lệ đúng 0\%
                trong benchmark.
