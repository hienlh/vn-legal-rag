...
Chương này trình bày các nền tảng lý thuyết được sử dụng trong luận văn và tổng hợp
các công trình liên quan. Phần~\ref{sec:cslt-truy-xuat} giới thiệu các phương pháp
truy xuất thông tin, từ phương pháp cơ bản đến đồ thị tri thức. Phần~\ref{sec:cong-trinh-lien-quan}
khảo sát các công trình nghiên cứu liên quan và xác định vị trí đóng góp của luận văn.

% ============================================================
\section{Các phương pháp truy xuất thông tin}
\label{sec:cslt-truy-xuat}

Phần này trình bày các phương pháp truy xuất thông tin được sử dụng trong luận văn,
bao gồm các phương pháp cơ bản (truy xuất thưa, truy xuất dày đặc, xếp hạng và
hợp nhất) và các phương pháp dựa trên đồ thị tri thức.

% ------------------------------------------------------------
\subsection{Các phương pháp cơ bản}
\label{subsec:phuong-phap-co-ban}
...
liên quan giảm dần. Các phương pháp truy xuất được chia thành hai nhóm chính: truy
xuất thưa (sparse retrieval) dựa trên so khớp thuật ngữ, và truy xuất dày đặc
(dense retrieval) dựa trên biểu diễn ngữ nghĩa.

\paragraph{Chỉ mục ngược.}
...
\paragraph{BM25 và TF-IDF.}
\label{subsec:bm25}
BM25 (Best Matching 25)~\cite{robertson2009bm25} là hàm xếp hạng dựa trên mô hình
xác suất, kết hợp tần suất thuật ngữ (TF), tần suất tài liệu ngược (IDF) với chuẩn
hóa độ dài tài liệu và bão hòa tần suất. BM25 cải tiến từ TF-IDF~\cite{manning2008ir}
--- phương pháp biểu diễn tài liệu bằng vector trọng số thuật ngữ --- bằng cách xử lý
hiệu quả hơn sự chênh lệch độ dài giữa các tài liệu. Cả hai phương pháp đều thuộc
nhóm truy xuất thưa (sparse retrieval): nhanh, dễ giải thích, và hiệu quả khi thuật
ngữ trong truy vấn trùng khớp với tài liệu. Tuy nhiên, truy xuất thưa thất bại trước
bài toán bất khớp từ vựng (vocabulary mismatch)~\cite{reimers2019sbert}: người dùng
hỏi \textit{``mở công ty cần gì''} trong khi văn bản quy định dùng thuật ngữ
\textit{``đăng ký thành lập doanh nghiệp''}. Hạn chế này thúc đẩy sự phát triển
của truy xuất dày đặc và truy xuất lai.

\paragraph{Các chỉ số đánh giá truy xuất.}
...
\paragraph{Embedding câu.}
\label{subsec:embedding-cau}
Truy xuất thưa dựa trên so khớp thuật ngữ không nắm bắt được tương đồng ngữ nghĩa
giữa các cách diễn đạt khác nhau của cùng một khái niệm. Truy xuất dày đặc (dense
retrieval) giải quyết vấn đề này bằng cách biểu diễn cả truy vấn và tài liệu dưới
dạng vector trong không gian liên tục, nơi khoảng cách vector phản ánh mức độ tương
đồng ngữ nghĩa.
...
\paragraph{Tìm kiếm vector với FAISS.}
\label{subsec:faiss}
...
\paragraph{Truy xuất lai.}
\label{subsec:truy-xuat-lai}
Truy xuất thưa giỏi so khớp thuật ngữ chính xác nhưng bỏ sót tương đồng ngữ nghĩa;
truy xuất dày đặc nắm bắt ngữ nghĩa nhưng có thể bỏ qua tín hiệu từ vựng quan
trọng~\cite{fan2024ragsurvey}. Truy xuất lai (hybrid retrieval) kết hợp cả hai
bằng cách chạy song song BM25 và tìm kiếm vector, sau đó hợp nhất kết quả thông qua
phương pháp như Reciprocal Rank Fusion (RRF). Cách tiếp cận này đã trở thành chuẩn
công nghiệp cho các hệ thống RAG~\cite{fan2024ragsurvey}, và là nền tảng mà kiến
trúc 3-Tier KG-RAG của luận văn mở rộng: thay vì chỉ kết hợp hai nguồn tín hiệu
(thưa và dày đặc), luận văn hợp nhất ba tầng truy xuất gồm duyệt cây cấu trúc,
tìm kiếm ngữ nghĩa hai cấp, và mở rộng đồ thị tri thức.

\paragraph{Xếp hạng lại.}
\label{subsec:xep-hang-lai}
Trong kiến trúc truy xuất hai giai đoạn (two-stage retrieval), giai đoạn đầu dùng
phương pháp nhanh (BM25 hoặc bi-encoder) để lọc $N$ ứng viên từ kho tài liệu lớn;
giai đoạn hai dùng cross-encoder --- mô hình xử lý đồng thời cặp (truy vấn, tài liệu)
--- để xếp hạng lại chính xác hơn~\cite{nogueira2020reranking}. Cross-encoder đạt
độ chính xác cao hơn bi-encoder nhưng có chi phí tính toán $O(N)$ theo số ứng viên,
nên chỉ áp dụng trên tập nhỏ đã lọc. Các nghiên cứu cho thấy reranking cải thiện
33--40\% độ chính xác với chỉ 120ms chi phí bổ sung. Luận văn không sử dụng
cross-encoder generic mà thay thế bằng cơ chế chuyên biệt miền: KG expansion tại
Tầng~3 đóng vai trò ``xếp hạng lại dựa trên tri thức'' --- bổ sung các điều khoản
liên quan thông qua quan hệ trong đồ thị tri thức thay vì chỉ dựa trên tương đồng
ngữ nghĩa bề mặt.

\paragraph{Truy xuất tăng cường bởi LLM.}
\label{subsec:truy-xuat-llm}
Sự phát triển của các mô hình ngôn ngữ lớn (LLM) đã mở ra paradigm mới trong truy
xuất thông tin: sử dụng LLM không chỉ để sinh câu trả lời mà còn để cải thiện chính
quá trình truy xuất~\cite{zhu2024llmir}. HyDE (Hypothetical Document Embeddings)~\cite{gao2023hyde}
tạo tài liệu giả định từ truy vấn rồi dùng embedding của tài liệu đó để tìm kiếm, giúp
thu hẹp khoảng cách ngữ nghĩa giữa truy vấn ngắn và tài liệu dài. Step-Back
Prompting trừu tượng hóa truy vấn cụ thể thành câu hỏi tổng quát hơn để mở rộng
phạm vi truy xuất. Các hệ thống Agentic RAG~\cite{asai2024selfrag} cho phép LLM
tự đánh giá và điều chỉnh chiến lược truy xuất qua nhiều vòng lặp. Tầng~1 của luận
văn thuộc paradigm này: sử dụng LLM để phân tích truy vấn và điều hướng trên cấu
trúc phân cấp của văn bản quy phạm (duyệt cây chương--điều), thay vì tìm kiếm
trực tiếp trên toàn bộ kho tài liệu.
...
% ------------------------------------------------------------
\subsection{Đồ thị tri thức và Ontology}
\label{subsec:kg-ontology}

\paragraph{Đồ thị tri thức.}
...
% ============================================================
\section{Các phương pháp truy vấn kiến thức pháp lý}
...
ngữ (XLM-R) và BERT tổng quát trên các tác vụ phân loại văn bản pháp luật
và rà soát hợp đồng.
