...
TF-IDF & 26.0 & 40.5 & 59.5 & 71.0 & 0.191 \\
\midrule
\multicolumn{6}{l}{\textit{RAG systems}} \\
LightRAG \cite{r5} & 22.5 & 34.0 & 48.0 & 53.5 & 0.147 \\
PageIndex \cite{r7} & \underline{52.5} & \underline{60.5} & \underline{65.0} & \underline{66.5} & \underline{0.364} \\
\midrule
\textbf{3-Tier (ours)} & \textbf{63.5} & \textbf{83.8} & \textbf{88.5} & \textbf{91.6} & \textbf{0.437} \\
\bottomrule
\end{tabular}
\end{table}

Our 3-Tier system reaches 83.8\% Hit@10, outperforming the best RAG baseline PageIndex by +23.3pp and the best traditional baseline TF-IDF by +43.3pp, with Recall@10\,=\,53.4\%. Among RAG systems, PageIndex (60.5\%) benefits from tree-based structural navigation but misses cross-reference articles outside the traversal path; LightRAG (34.0\%) underperforms even TF-IDF because its entity extraction produces English-language entities from Vietnamese input, resulting in query--entity mismatch. Traditional methods (BM25/TF-IDF, $\sim$36--41\%) lack structural awareness; semantic search (15.5\%) fails on Vietnamese legal terminology.
...
\midrule
Enterprise Law & 150 & 28.3 & 60.3 & 83.7 & 88.4 & 90.0 & 0.425 \\
Traffic Safety Law & 50 & 32.0 & 73.2 & 84.0 & 88.8 & 96.4 & 0.473 \\
\midrule
\textbf{Combined} & \textbf{200} & \textbf{29.2} & \textbf{63.5} & \textbf{83.8} & \textbf{88.5} & \textbf{91.6} & \textbf{0.437} \\
\bottomrule
\end{tabular}
\end{table}

Both domains achieve comparable Hit@10 (enterprise 83.7\%, traffic 84.0\%), with traffic law showing higher Hit@30 (96.4\% vs.\ 90.0\%) because its penalty articles map directly to specific violations and are reliably ranked within the top~30. Enterprise law involves more cross-decree references. No domain-specific tuning was applied; identical pipeline, parameters, and RRF weights serve both domains.

\subsection{Analysis}

\subsubsection{Performance by Query Category}

Hit@10 ranges from 53.8\% to 96.7\% across categories. Capital contributions (96.7\%), dissolution (94.5\%), and traffic rules (92.5\%) score highest, as these map cleanly to localized article clusters. Company types (53.8\%) and other enterprise (61.4\%) score lowest because answers span multiple decrees outside the primary law.
